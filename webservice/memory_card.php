...
function listCard(){    
    ...
    foreach($res['empRecords'] as $row){
        ...
        $onclick = 'onclick="siparsFramework.takeCharge(\''.$row['ID'].'\',\'memory_card\', \'load\', \'\', \'form-edit-codice\'  )"';
        ...
        if($row['CANCELLATO']==0){
            $onclickDisable = 'onclick="siparsFramework.takeChargeConfirm(\''.$row['ID'].'\',\'memory_card\', \'delete\', \'#ListCard\' )"';
            $colorBtnDisable = 'btn-success';
            $iconBtnDisable = 'fas fa-toggle-on';
            
        } else {
            $onclickDisable = 'onclick="siparsFramework.takeChargeConfirm(\''.$row['ID'].'\',\'memory_card\', \'riattiva\', \'#ListCard\' )"';
