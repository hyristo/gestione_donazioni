...
function listTesti(){   
    ...
    foreach($res['empRecords'] as $row){
        ...
        $onclick = 'onclick="siparsFramework.takeCharge(\''.$row['ID'].'\',\'biblioteca\', \'load\', \'\', \'form-edit-codice\'  )"';
        ...
        if($row['CANCELLATO']==0){
            $onclickDisable = 'onclick="siparsFramework.takeChargeConfirm(\''.$row['ID'].'\',\'biblioteca\', \'delete\', \'#ListTesti\' )"';
            $colorBtnDisable = 'btn-success';
            $iconBtnDisable = 'fas fa-toggle-on';
            
        } else {
            $onclickDisable = 'onclick="siparsFramework.takeChargeConfirm(\''.$row['ID'].'\',\'biblioteca\', \'riattiva\', \'#ListTesti\' )"';
